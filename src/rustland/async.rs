...
use wmstate::{WM_STATE, ACTIVE_TRANSITIONS, PENDING_JOBS, FINALIZED_JOBS};
...
/*
    Subroutines for asynchronous processing 
*/
...
pub fn schedule_tcp_routine(){
    spawn(||{
        if let Some(ref tcp_socket) = *TCP_SOCKET{
            if let Ok(ref socket_instance) = tcp_socket.try_lock(){
